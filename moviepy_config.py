...
def configure_moviepy_gpu():
    """
    Configure MoviePy for GPU acceleration using FFmpeg.
    ...
    try:
        if gpu_config.ffmpeg_gpu_support:
            # Set FFmpeg parameters for GPU acceleration
            gpu_params = gpu_config.get_ffmpeg_gpu_params()
            if gpu_params:
                # Note: MoviePy doesn't have direct GPU parameter support
                # GPU acceleration will be handled at the FFmpeg command level
                print("GPU acceleration available for FFmpeg operations")
                return True
        
        print("GPU acceleration not available for FFmpeg")
        return False
        
    except Exception as e:
        print(f"Warning: Could not configure MoviePy GPU acceleration: {e}")
        return False
