...
# Import the centralized MoviePy configuration and GPU config
import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
from moviepy_config import configure_moviepy_temp_dir
from gpu_config import gpu_config, get_yolo_device, get_pyannote_device, optimize_model, clear_gpu_cache, write_video_with_gpu_acceleration

# Configure MoviePy temporary directory for speaker tracking
configure_moviepy_temp_dir("/app/tmp")
...
def initialize_models():
    ...
    try:
        ...
        yolo_model = optimize_model(yolo_model, "yolo")
    ...
    try:
        ...
        if not hf_token:
            ...
            device = get_pyannote_device()
            print(f"Loading PyAnnote pipeline on device: {device}")
            ...
            if gpu_config.cuda_available:
                ...
                print(f"PyAnnote pipeline moved to {device}")
    except Exception as e:
...
def apply_dynamic_cropping_tracking(video_path, output_path, active_speaker_segments, fps, professional_camera=True):
    ...
    try:
        ...
        # Write the final video with professional quality settings and GPU acceleration
        quality_level = 'ultra' if professional_camera else 'high'
