...
def serve_processed_file(filename):
    ...
    try:
        ...
        # Use send_file with explicit MIME type for better browser compatibility
        return send_file(f'local_processed/{decoded_filename}', mimetype=mime_type)
    except Exception as e:
...
def reprocess_video():
    ...
    try:
        ...
        # Generate response with proper URL handling
        # --- R2 Uploading ---
        final_video_url = None
        final_transcription_url = None
        final_original_video_url = None
        ...
        if USE_R2_STORAGE and r2_client:
            print("Uploading reprocessed artifacts to R2...")
            try:
                # Upload processed video
                processed_video_path = os.path.join(PROCESSED_FOLDER, output_filename)
                if os.path.exists(processed_video_path):
                    r2_client.upload_file(processed_video_path, R2_VIDEO_BUCKET, output_filename)
                    final_video_url = f"{R2_PUBLIC_URL}/{output_filename}"
                    print(f"Successfully uploaded reprocessed video to R2: {final_video_url}")
                    os.remove(processed_video_path) # Clean up local file

                # Upload transcription
                transcription_file_path = os.path.join(PROCESSED_FOLDER, new_transcription_filename)
                if os.path.exists(transcription_file_path):
                    r2_client.upload_file(transcription_file_path, R2_VIDEO_BUCKET, new_transcription_filename)
                    final_transcription_url = f"{R2_PUBLIC_URL}/{new_transcription_filename}"
                    print(f"Successfully uploaded reprocessed transcription to R2: {final_transcription_url}")
                    os.remove(transcription_file_path)

                # For original video URL, use the video_path if it's already a URL, otherwise upload
                if video_path.startswith(('http://', 'https://')):
                    final_original_video_url = video_path
                else:
                    original_video_filename = os.path.basename(video_path)
                    original_video_path = os.path.join(PROCESSED_FOLDER, original_video_filename)
                    if os.path.exists(original_video_path):
                        r2_client.upload_file(original_video_path, R2_VIDEO_BUCKET, original_video_filename)
                        final_original_video_url = f"{R2_PUBLIC_URL}/{original_video_filename}"
                ...
                print(f"ERROR: Failed to upload reprocessed files to R2: {e}")
                # Fallback to local URLs if R2 upload fails
                final_video_url = url_for('get_processed_file', filename=output_filename, _external=True)
                final_transcription_url = url_for('get_transcription', filename=new_transcription_filename, _external=True)
                final_original_video_url = video_path if video_path.startswith(('http://', 'https://')) else url_for('get_processed_file', filename=os.path.basename(video_path), _external=True)
        else:
            # Generate local URLs if not using R2
            final_video_url = url_for('get_processed_file', filename=output_filename, _external=True)
            final_transcription_url = url_for('get_transcription', filename=new_transcription_filename, _external=True)
...
def edit_captions():
    ...
    try:
        ...
        # Generate URLs with proper R2 handling
        final_video_url = None
        final_transcription_url = None
        ...
        if USE_R2_STORAGE and r2_client:
            print("Uploading edited video to R2...")
            try:
                # Upload edited video
                edited_video_path = os.path.join(app.config['PROCESSED_FOLDER'], edited_filename)
                if os.path.exists(edited_video_path):
                    r2_client.upload_file(edited_video_path, R2_VIDEO_BUCKET, edited_filename)
                    final_video_url = f"{R2_PUBLIC_URL}/{edited_filename}"
                    print(f"Successfully uploaded edited video to R2: {final_video_url}")
                    os.remove(edited_video_path) # Clean up local file

                # Upload transcription
                if os.path.exists(transcription_path):
                    r2_client.upload_file(transcription_path, R2_VIDEO_BUCKET, transcription_filename)
                    final_transcription_url = f"{R2_PUBLIC_URL}/{transcription_filename}"
                    print(f"Successfully uploaded edited transcription to R2: {final_transcription_url}")
                    os.remove(transcription_path)

            except ClientError as e:
                print(f"ERROR: Failed to upload edited files to R2: {e}")
                # Fallback to local URLs if R2 upload fails
                final_video_url = url_for('get_processed_file', filename=edited_filename, _external=True)
                final_transcription_url = url_for('get_transcription', filename=transcription_filename, _external=True)
        else:
            # Generate local URLs if not using R2
            final_video_url = url_for('get_processed_file', filename=edited_filename, _external=True)
    ...
    try:
        ...
        if USE_R2_STORAGE and r2_client:
            ...
            try:
                ...
                print(f"ERROR: Failed to upload edited video to R2: {e}")
                # Fallback to local URLs if R2 upload fails
                final_video_url = url_for('get_processed_file', filename=edited_filename, _external=True)
        else:
            # Generate local URLs if not using R2
