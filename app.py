...
@app.route('/api/regenerate-video', methods=['POST', 'OPTIONS'])
@log_route_details
def regenerate_video_with_custom_transcription():
    """API endpoint to regenerate a video with custom transcription (edited words)"""
    from flask import jsonify, request
    ...
    try:
        ...
        custom_transcription_json = request.form.get('custom_transcription')
        template_id = request.form.get('template', 'minimal_white')
        filter_profanity = request.form.get('filter_profanity', 'false').lower() == 'true'
        enable_speaker_tracking = request.form.get('enable_speaker_tracking', 'false').lower() == 'true'
        
        if not filename or not custom_transcription_json:
            return jsonify({'error': 'Missing required parameters: filename and custom_transcription'}), 400
        
        # Parse the custom transcription
        try:
            custom_transcription = json.loads(custom_transcription_json)
            segments = custom_transcription.get('segments', [])
        except json.JSONDecodeError:
            return jsonify({'error': 'Invalid JSON in custom_transcription parameter'}), 400
        
        if not segments:
            return jsonify({'error': 'No segments found in custom transcription'}), 400
        
        print(f"Regenerating video with custom transcription: {filename}")
        print(f"Template: {template_id}, Segments: {len(segments)}")
        ...
        possible_paths = [
            ...
            os.path.join(PROCESSED_FOLDER, f"original_{filename}"),
        ]
        ...
        if not original_video_path:
            return jsonify({'error': f'Original video file not found: {filename}'}), 404
        
        # Generate output filename
        timestamp = int(time.time())
        base_name = os.path.splitext(filename)[0]
        output_filename = f"edited_{base_name}_{timestamp}.mp4"
        output_path = os.path.join(PROCESSED_FOLDER, output_filename)
        
        # Create temporary video copy
        temp_dir = "/app/tmp"
        os.makedirs(temp_dir, exist_ok=True)
        temp_video_path = os.path.join(temp_dir, f"temp_{uuid.uuid4().hex}_{filename}")
        
        # Copy original video to temp location
        shutil.copy2(original_video_path, temp_video_path)
        
        # Split segments into phrases for caption generation
        phrases = split_into_phrases(segments, max_words=3)
        
        # Create transcription file for the regenerated video
        transcription_filename = output_filename.rsplit('.', 1)[0] + '.json'
        transcription_path = os.path.join(PROCESSED_FOLDER, transcription_filename)
        ...
        # Add captions to the video with the custom transcription
        # Always add watermark for regenerated videos (can be changed based on user plan)
        add_watermark = True  # You can modify this based on user plan if needed
        user_plan = "free"  # Default plan, can be passed as parameter if needed
        ...
        print(f"Successfully regenerated video: {output_filename}")
        return jsonify(response)
        
    except Exception as e:
        print(f"Error regenerating video: {str(e)}")
        return jsonify({'error': f'Failed to regenerate video: {str(e)}'}), 500
