...
from flask import Flask, request, render_template, send_from_directory, redirect, url_for, jsonify, Response, session, current_app
...
def add_cors_headers(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Convert to Flask Response object if it isn't already
        if not isinstance(response, Response):
            if isinstance(response, tuple):
                # Handle (data, status_code) tuple responses
...
@app.route('/processed/<path:filename>', methods=['GET', 'OPTIONS'])
@log_route_details
def get_processed_file(filename):
    ...
    if request.method == 'OPTIONS':
        ...
        resp.headers.add('Access-Control-Allow-Methods', 'GET,OPTIONS')
        resp.headers.add('Access-Control-Expose-Headers', 'Content-Length,Content-Range')
        return resp
    ...
    try:
        ...
        response = send_from_directory(app.config['PROCESSED_FOLDER'], decoded_filename, as_attachment=download)
        # Add CORS headers
        response.headers.add('Access-Control-Allow-Origin', '*')
        response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization,Range')
        response.headers.add('Access-Control-Allow-Methods', 'GET,OPTIONS')
...
@app.route('/local_processed/<path:filename>', methods=['GET', 'OPTIONS'])
@log_route_details
def serve_processed_file(filename):
    """Serve files directly from the local_processed directory"""
    # Handle CORS preflight request
    if request.method == 'OPTIONS':
        resp = jsonify({'status': 'ok'})
        resp.headers.add('Access-Control-Allow-Origin', '*')
    ...
    try:
        ...
        response = send_from_directory('local_processed', decoded_filename)
...
@app.route('/local_uploads/<path:filename>', methods=['GET', 'OPTIONS'])
@log_route_details
def serve_uploaded_file(filename):
    """Serve files directly from the local_uploads directory"""
    # Handle CORS preflight request
    if request.method == 'OPTIONS':
        resp = jsonify({'status': 'ok'})
        resp.headers.add('Access-Control-Allow-Origin', '*')
    ...
    try:
        # URL decode the filename to handle spaces and special characters
        decoded_filename = urllib.parse.unquote(filename)
        print(f"Serving local_uploads file: {decoded_filename}")
        
        response = send_from_directory('local_uploads', decoded_filename)
        ...
        print(f"Error serving local_uploads file: {str(e)}")
        return f"Error serving file: {str(e)}", 500

@app.route('/view/<filename>', methods=['GET', 'OPTIONS'])
@log_route_details
def view_processed_file(filename):
    ...
    response = send_from_directory(app.config['PROCESSED_FOLDER'], filename, as_attachment=False)
    # Add CORS headers for video files
    response.headers.add('Access-Control-Allow-Origin', '*')
    response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization,Range')
    response.headers.add('Access-Control-Allow-Methods', 'GET,OPTIONS')
