        ...
        for word_idx, word in enumerate(words):
            ...
            # Add outline for templates that need it
                ...
                if template.lower() == "yellow_impact":
                    ...
                    outline_color = (0, 0, 0, 255)  # Black outline for Aurora Bold (Green Bold)
                    outline_thickness = 3  # 3px black outline for Aurora Bold
                elif template.lower() == "meme_orange":
                ...
                # Calculate bounce offset for Aurora Bold (green_bold) template
                bounce_y_offset = 0
                if template.lower() == "green_bold" and is_highlighted:
                    # Create a bounce animation effect for highlighted words
                    animation_phase = (time.time() * 4) % 1.0  # Cycles through 0 to 1 every 1/4 second
                    
                    # Create a bounce effect using a sine wave with easing
                    if animation_phase < 0.5:
                        # Rising phase - accelerate upward
                        bounce_progress = animation_phase * 2  # 0 to 1 in first half
                        bounce_y_offset = -15 * (1 - (1 - bounce_progress) ** 2)  # Ease-out curve, max -15px
                    else:
                        # Falling phase - decelerate downward
                        bounce_progress = (animation_phase - 0.5) * 2  # 0 to 1 in second half
                        bounce_y_offset = -15 * (bounce_progress ** 2)  # Ease-in curve, back to 0
                ...
                for dx, dy in [(-outline_thickness, -outline_thickness), 
                              ...
                              (-outline_thickness, 0),
                              (outline_thickness, 0)]:
                    draw.text((current_x + dx, y_position + line_idx * line_height + dy + bounce_y_offset), 
                             word_with_space, font=font, fill=outline_color)
            ...
            if template.lower() == "trendy_gradient":
                ...
                if '*' in word or (is_current_word and is_profane):
                    ...
                    # For Aurora Bold (green_bold) template, add bounce animation for highlighted words
                    elif template.lower() == "green_bold" and is_highlighted:
                        # Create a bounce animation effect for highlighted words
                        # Calculate the animation phase based on the current time
                        animation_phase = (time.time() * 4) % 1.0  # Cycles through 0 to 1 every 1/4 second
                        
                        # Create a bounce effect using a sine wave with easing
                        # Use a quadratic ease-out function for more natural bounce
                        if animation_phase < 0.5:
                            # Rising phase - accelerate upward
                            bounce_progress = animation_phase * 2  # 0 to 1 in first half
                            bounce_offset = -15 * (1 - (1 - bounce_progress) ** 2)  # Ease-out curve, max -15px
                        else:
                            # Falling phase - decelerate downward
                            bounce_progress = (animation_phase - 0.5) * 2  # 0 to 1 in second half
                            bounce_offset = -15 * (bounce_progress ** 2)  # Ease-in curve, back to 0
                        
                        # Apply the bounce offset to the y position
                        bounce_y = y_position + line_idx * line_height + bounce_offset
                        
                        # Draw the main text with bounce animation
                        draw.text((current_x, bounce_y), word_with_space, font=font, fill=word_color)
                    else:
