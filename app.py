...
app.config['TIMEOUT'] = 1800  # 30 minutes timeout for long video processing
...
def api_process():
    ...
    try:
        ...
        if USE_R2_STORAGE and r2_client:
            ...
            try:
                ...
                if os.path.exists(transcription_file_path):
                    ...
                    try:
                        ...
                        print(f"Warning: Could not add metadata to transcription: {e}")
                    
                    r2_transcription_key = f"{user_prefix}{transcription_filename}"
                    r2_client.upload_file(transcription_file_path, R2_VIDEO_BUCKET, r2_transcription_key)
                    final_transcription_url = f"{R2_PUBLIC_URL}/{r2_transcription_key}"
                ...
                if os.path.exists(original_video_path):
                    r2_original_key = f"{user_prefix}{original_video_filename}"
                    r2_client.upload_file(original_video_path, R2_VIDEO_BUCKET, r2_original_key)
                    final_original_video_url = f"{R2_PUBLIC_URL}/{r2_original_key}"
            ...
            # Organize files by user in local storage
            if user_id:
                user_folder = os.path.join(PROCESSED_FOLDER, f"users/{user_id}")
                os.makedirs(user_folder, exist_ok=True)
                
                # Move files to user-specific folder
                for filename in [output_filename, transcription_filename, original_video_filename]:
                    src_path = os.path.join(PROCESSED_FOLDER, filename)
                    dst_path = os.path.join(user_folder, filename)
                    if os.path.exists(src_path):
                        shutil.move(src_path, dst_path)
                        print(f"Moved {filename} to user folder")
                
                # Add metadata to transcription file
                transcription_path = os.path.join(user_folder, transcription_filename)
                ...
                final_video_url = url_for('get_user_video', user_id=user_id, filename=encoded_output_filename, _external=True)
                final_transcription_url = url_for('get_user_transcription', user_id=user_id, filename=encoded_transcription_filename, _external=True)
                final_original_video_url = url_for('get_user_video', user_id=user_id, filename=encoded_original_filename, _external=True)
            else:
        ...
        # Create the JSON response with explicit CORS headers
        json_response = jsonify(response)
        json_response.headers.add('Access-Control-Allow-Origin', request.headers.get('Origin', '*'))
        json_response.headers.add('Access-Control-Allow-Methods', 'POST, OPTIONS')
        json_response.headers.add('Access-Control-Allow-Headers', 'Content-Type')
        
        print(f"JSON response created with headers: {dict(json_response.headers)}")
...
@log_route_details
def api_list_videos():
    """API endpoint to list user's processed videos"""
    if request.method == 'OPTIONS':
        return jsonify({'status': 'ok'})
    ...
    try:
        ...
        if USE_R2_STORAGE and r2_client:
            try:
                # List objects in the R2 bucket with user prefix
                ...
                if 'Contents' in response:
                    for obj in response['Contents']:
                        ...
                        # Skip non-video files (transcriptions, etc.)
                        if not key.endswith(('.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv')):
                            continue
                            
                        # Extract video info from the key
                        filename = key.split('/')[-1]
                        
                        # Skip original videos (they have 'original_' prefix)
                        ...
                        # Try to get additional metadata from transcription file
                        transcription_key = key.replace('.mp4', '_transcription.json')
                        try:
                            transcription_response = r2_client.get_object(
                                Bucket=R2_VIDEO_BUCKET,
                                Key=transcription_key
                            )
                            transcription_data = json.loads(transcription_response['Body'].read())
                            
                            # Add metadata from transcription
                            if 'template' in transcription_data:
                                video_info['template'] = transcription_data['template']
                            if 'resolution' in transcription_data:
                                video_info['resolution'] = transcription_data['resolution']
                            if 'duration' in transcription_data:
                                video_info['duration'] = transcription_data['duration']
                                
                            video_info['transcriptionUrl'] = f"{R2_PUBLIC_URL}/{transcription_key}"
                            video_info['hasTranscription'] = True
                            
                        except ClientError:
                            # Transcription file doesn't exist, that's okay
                            video_info['hasTranscription'] = False
                ...
                return jsonify({
                    'success': False,
                    'error': 'Failed to fetch videos from storage'
                }), 500
        else:
            # Fallback to local storage
            user_folder = os.path.join(PROCESSED_FOLDER, f"users/{user_id}")
            if os.path.exists(user_folder):
                for filename in os.listdir(user_folder):
                    if filename.endswith(('.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv')):
                        if filename.startswith('original_'):
                            continue
                        ...
                        video_info = {
                            'id': filename.replace('.mp4', ''),
                            'filename': filename,
                            'videoUrl': url_for('get_user_video', user_id=user_id, filename=filename, _external=True),
                        ...
                        transcription_filename = filename.replace('.mp4', '_transcription.json')
                        transcription_path = os.path.join(user_folder, transcription_filename)
                        ...
                        if os.path.exists(transcription_path):
                            try:
                                ...
                                video_info['transcriptionUrl'] = url_for('get_user_transcription', user_id=user_id, filename=transcription_filename, _external=True)
                                video_info['hasTranscription'] = True
                            except:
                                video_info['hasTranscription'] = False
                        else:
        ...
        return jsonify({
            ...
            'videos': videos,
            'count': len(videos),
            'storage_type': 'r2' if USE_R2_STORAGE else 'local'
        })
...
def api_delete_video(video_id):
    """API endpoint to delete a specific video"""
    if request.method == 'OPTIONS':
        return jsonify({'status': 'ok'})
    ...
    try:
        ...
        if USE_R2_STORAGE and r2_client:
            try:
                ...
                files_to_delete = [
                    f"{user_prefix}{video_id}.mp4",
                    f"{user_prefix}{video_id}_transcription.json",
                    f"{user_prefix}original_{video_id}.mp4"
                ]
                
                deleted_files = []
                for file_key in files_to_delete:
                    try:
                        r2_client.delete_object(Bucket=R2_VIDEO_BUCKET, Key=file_key)
                        deleted_files.append(file_key)
                        print(f"Deleted {file_key} from R2")
                    except ClientError as e:
                        if e.response['Error']['Code'] != 'NoSuchKey':
            ...
            files_to_delete = [
                f"{video_id}.mp4",
                f"{video_id}_transcription.json",
                f"original_{video_id}.mp4"
            ]
...
@app.route('/api/users/<user_id>/videos/<filename>')
@log_route_details
def get_user_video(user_id, filename):
    """Serve user-specific video files"""
    try:
        decoded_filename = urllib.parse.unquote(filename)
        ...
        if USE_R2_STORAGE and r2_client:
            try:
                # Generate a presigned URL for the R2 object
                user_key = f"users/{user_id}/{decoded_filename}"
                presigned_url = r2_client.generate_presigned_url(
                    'get_object',
                    Params={'Bucket': R2_VIDEO_BUCKET, 'Key': user_key},
                    ExpiresIn=3600  # URL expires in 1 hour
                )
                return redirect(presigned_url)
            except ClientError as e:
        ...
        print(f"Error serving user video: {str(e)}")
        return f"Error serving video: {str(e)}", 500

@app.route('/api/users/<user_id>/transcriptions/<filename>')
@log_route_details
def get_user_transcription(user_id, filename):
    """Serve user-specific transcription files"""
    try:
        decoded_filename = urllib.parse.unquote(filename)
        ...
        if USE_R2_STORAGE and r2_client:
            try:
                ...
                user_key = f"users/{user_id}/{decoded_filename}"
                response = r2_client.get_object(Bucket=R2_VIDEO_BUCKET, Key=user_key)
                transcription_data = response['Body'].read()
                ...
                print(f"Error getting transcription from R2: {e}")
                return f"Transcription not found: {str(e)}", 404
        else:
            # Serve from local storage
            user_folder = os.path.join(PROCESSED_FOLDER, f"users/{user_id}")
            file_path = os.path.join(user_folder, decoded_filename)
        ...
        print(f"Error serving user transcription: {str(e)}")
        return f"Error serving transcription: {str(e)}", 500
