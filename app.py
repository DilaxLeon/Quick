...
import whisper
import numpy as np
from moviepy.editor import VideoFileClip, TextClip, ImageClip, CompositeVideoClip, AudioFileClip, concatenate_audioclips, CompositeAudioClip
...
def render_pil_caption(params):
    """Render a caption using PIL instead of TextClip
    
    Args:
        params: Dictionary containing all parameters needed to render the caption
        
    Returns:
        A dictionary with the rendered caption image and timing information
    """
    video_size = params["video_size"]
    template = params["template"]
    font_size = params["font_size"]
    position_y_percentage = params["position_y_percentage"]
    start_time = params["start_time"]
    duration = params["duration"]
    
    # Create a transparent image with the video dimensions
    img = Image.new('RGBA', video_size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    # Load font based on template
    font_path = get_font_for_template(template)
    try:
    ...
    # Get text and handle word-level highlighting
    if params["type"] == "word":
        text = params["word_data"]["word"]
        is_profane = params["word_data"].get("is_profane", False)
        
        # Draw text
        text_width, text_height = draw.textbbox((0, 0), text, font=font)[2:]
        text_position = ((video_size[0] - text_width) // 2, int(video_size[1] * position_y_percentage) - text_height // 2)
        
        # Draw text with appropriate color
        text_color = (255, 255, 255, 255)  # Default white
        if is_profane:
            text_color = (255, 0, 0, 255)  # Red for profane words
            
        draw.text(text_position, text, font=font, fill=text_color)
        
    elif params["type"] == "word_highlight":
        text = params["word_data"]["word"]
        is_profane = params["word_data"].get("is_profane", False)
        ...
        if highlight_color and highlight_color != (255, 255, 255, 255):
            ...
            # Use a slightly transparent version of the highlight color
            r, g, b, a = highlight_color
            bg_color = (r, g, b, min(a, 180))  # Cap alpha at 180 for better readability
            
            draw.rectangle(highlight_box, fill=bg_color)
        
        # Draw text with appropriate color
        text_color = (255, 255, 255, 255)  # Default white
        if is_profane:
            text_color = (255, 0, 0, 255)  # Red for profane words
        elif template.lower() in ["mrbeast", "green_bold"] and highlight_color:
            text_color = highlight_color  # Use highlight color for text in certain templates
            
        draw.text(text_position, text, font=font, fill=text_color)
        
        # Add outline for certain templates
        if template.lower() in ["vlogger_bold", "meme_orange", "film_noir"]:
            # Create a temporary image for the outline effect
            outline_img = Image.new('RGBA', video_size, (0, 0, 0, 0))
            outline_draw = ImageDraw.Draw(outline_img)
            
            # Draw text multiple times with slight offsets for outline
            outline_color = (0, 0, 0, 255)  # Black outline
            outline_size = max(1, int(font_size * 0.05))  # Scale outline with font size
            
            for dx, dy in [(-outline_size, -outline_size), (-outline_size, outline_size), 
                           (outline_size, -outline_size), (outline_size, outline_size)]:
                outline_draw.text((text_position[0] + dx, text_position[1] + dy), 
                                 text, font=font, fill=outline_color)
            
            # Apply slight blur to the outline
            outline_img = outline_img.filter(ImageFilter.GaussianBlur(radius=outline_size/2))
            
            # Composite the outline behind the text
            img = Image.alpha_composite(outline_img, img)
            
    elif params["type"] == "phrase":
        text = params["text"]
        ...
        line_height = int(font_size * 1.2)  # Add 20% spacing between lines
        total_height = len(lines) * line_height
        
        # Draw each line centered
        y_offset = int(video_size[1] * position_y_percentage) - total_height // 2
        
        for line in lines:
            text_width = draw.textbbox((0, 0), line, font=font)[2]
            text_position = ((video_size[0] - text_width) // 2, y_offset)
            
            # Draw text with white color
            draw.text(text_position, line, font=font, fill=(255, 255, 255, 255))
...
def get_font_for_template(template):
    """Get the appropriate font path for a given template"""
    # Default font path
    default_font = os.path.abspath(os.path.join('Fonts', 'SpiegelSans.otf'))
    
    # Map templates to fonts
    template_fonts = {
        # Word by Word Templates
        "yellow_impact": os.path.abspath(os.path.join('Fonts', 'Impact.ttf')),
        "bold_white": os.path.abspath(os.path.join('Fonts', 'Montserrat-Bold.ttf')),
        "bold_green": os.path.abspath(os.path.join('Fonts', 'Montserrat-Bold.ttf')),
        
        # Classic/Minimal Templates
        "minimalist_sans": os.path.abspath(os.path.join('Fonts', 'OpenSans-Regular.ttf')),
        "minimal_white": os.path.abspath(os.path.join('Fonts', 'SpiegelSans.otf')),
        "elegant_pink": os.path.abspath(os.path.join('Fonts', 'Anton-Regular.ttf')),
        
        # Entertainment/Vlogger Templates
        "mrbeast": os.path.abspath(os.path.join('Fonts', 'Poppins-Bold.ttf')),
        "vlogger_bold": os.path.abspath(os.path.join('Fonts', 'Helvetica-Rounded-Bold.otf')),
        "creator_clean": os.path.abspath(os.path.join('Fonts', 'Montserrat-SemiBold.ttf')),
        "reaction_pop": os.path.abspath(os.path.join('Fonts', 'LeagueSpartan-Bold.otf')),
        
        # Social Media Templates
        "tiktok": os.path.abspath(os.path.join('Fonts', 'Poppins-SemiBold.ttf')),
        "insta_story": os.path.abspath(os.path.join('Fonts', 'PoetsenOne-Regular.ttf')),
        
        # Educational/Informative Templates
        "explainer_pro": os.path.abspath(os.path.join('Fonts', 'Helvetica-Rounded-Bold.otf')),
        "science_journal": os.path.abspath(os.path.join('Fonts', 'Geist-Black.otf')),
        
        # Gaming Templates
        "gaming_neon": os.path.abspath(os.path.join('Fonts', 'Exo2-VariableFont_wght.ttf')),
        
        # Cinematic/Film Templates
        "film_noir": os.path.abspath(os.path.join('Fonts', 'Anton-Regular.ttf')),
        "cinematic_quote": os.path.abspath(os.path.join('Fonts', 'Proxima Nova Alt Condensed Black Italic.otf')),
        
        # Comedy/Memes Templates
        "meme_orange": os.path.abspath(os.path.join('Fonts', 'LuckiestGuy.ttf')),
        
        # Trendy/Viral Templates
        "green_bold": os.path.abspath(os.path.join('Fonts', 'Uni Sans Heavy.otf')),
        "trendy_gradient": os.path.abspath(os.path.join('Fonts', 'LuckiestGuy.ttf')),
    }
    
    # Return the appropriate font path or default if not found
    return template_fonts.get(template.lower(), default_font)
...
def add_captions_to_video(video_path, output_path, phrases, template="minimal_white", highlight_color=None, add_watermark=False, user_plan="free"):
    ...
    # Use consistent font size for all videos based on template
    font_size_percentage = 0.05  # Default font size percentage
    ...
    # Define custom color mapping for highlight colors
    custom_colors = {
    ...
    # Prepare parameters for all captions that need to be rendered
    caption_params = []
    ...
    for phrase in phrases:
        ...
        if template.lower() in ["yellow_impact", "bold_white", "bold_green"] and "word_timestamps" in phrase and len(phrase["word_timestamps"]) > 0:
            # For Word by Word templates, we'll create a clip for each word individually
            for word_data in phrase["word_timestamps"]:
                ...
                caption_params.append({
                    ...
                    "position_y_percentage": 0.6,  # 60% from the top
                    "template": template,
        ...
        if "word_timestamps" in phrase and len(phrase["word_timestamps"]) > 0:
            ...
            for word_data in phrase["word_timestamps"]:
                ...
                # Add parameters for this caption to be rendered in parallel
                caption_params.append({
                    "type": "word_highlight",
                    "word_data": word_data,
                    "start_time": word_start,
                    ...
                    "position_y_percentage": 0.8 if is_vertical else 0.85,  # 80% for vertical, 85% for horizontal
                    "template": template,
                    "video_size": video_size,
                    "is_profane": is_profane
                })
    ...
    # Use ThreadPoolExecutor to render captions in parallel
    print(f"Rendering {len(caption_params)} captions in parallel using PIL...")
    start_render_time = time.time()
    
    # Determine the optimal number of workers based on CPU cores
    max_workers = min(os.cpu_count() or 4, 8)  # Use at most 8 workers to avoid memory issues
    
    # Render all captions in parallel
    rendered_captions = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Process all captions in parallel and collect results
        rendered_captions = list(executor.map(render_pil_caption, caption_params))
    
    end_render_time = time.time()
    print(f"Parallel rendering completed in {end_render_time - start_render_time:.2f} seconds")
    
    # Create MoviePy clips from the rendered PIL images
    caption_clips = []
    for caption in rendered_captions:
        # Create a clip from the numpy array
        img_clip = ImageClip(caption["image"])
        # Set the timing and duration
        img_clip = img_clip.set_start(caption["start_time"]).set_duration(caption["duration"])
