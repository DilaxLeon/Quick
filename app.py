...
def get_quality_info(user_plan, video_size=None):
    """Get quality information based on user plan and video dimensions
    
    Args:
        user_plan: User's subscription plan ('free', 'basic', or 'pro')
        video_size: Optional tuple of (width, height) for resolution info
    ...
    # Add actual resolution if provided
    if video_size:
        width, height = video_size
        quality_info['actual_resolution'] = f"{width}x{height}"
        
        # Determine actual quality tier based on resolution
        if height >= 2160:
            quality_info['actual_tier'] = '4K'
        elif height >= 1080:
            quality_info['actual_tier'] = '1080p'
        elif height >= 720:
            quality_info['actual_tier'] = '720p'
        else:
            quality_info['actual_tier'] = 'SD'
...
def create_caption_clip(text, video_size, duration, font_size=40, template="minimal_white", current_word=None, current_word_color=None, is_profane=False, custom_text_color=None, custom_highlight_color=None, custom_font=None):
    ...
    try:
        # Try to enable anti-aliasing and high-quality rendering
...
def api_process():
    ...
    try:
        ...
        # Get video dimensions and quality information
        video_size = None
        processed_video_path = os.path.join(PROCESSED_FOLDER, output_filename)
        ...
        try:
            if os.path.exists(processed_video_path):
                from moviepy.editor import VideoFileClip
                with VideoFileClip(processed_video_path) as clip:
                    video_size = clip.size
                    print(f"Final video dimensions: {video_size[0]}x{video_size[1]}")
        except Exception as e:
            print(f"Could not get video dimensions: {e}")
        
        # Get quality information
        quality_info = get_quality_info(user_plan, video_size)
        ...
        try:
            if os.path.exists(output_path):
                ...
                with VideoFileClip(output_path) as clip:
                    video_size = clip.size
                    print(f"Edited video dimensions: {video_size[0]}x{video_size[1]}")
        except Exception as e:
            print(f"Could not get edited video dimensions: {e}")
        
        # Get quality information
        quality_info = get_quality_info(user_plan, video_size)
        print(f"Edit captions quality info: {quality_info}")
...
def edit_video():
    ...
    try:
        ...
        # Get quality information (edit_video uses basic settings, so assume free plan quality)
        quality_info = get_quality_info('free', video_size)
        print(f"Edit video quality info: {quality_info}")
