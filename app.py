...
def create_caption_clip(text, video_size, duration, font_size=40, template="minimal_white", current_word=None, current_word_color=None, is_profane=False, custom_text_color=None, custom_highlight_color=None, custom_font=None):
    ...
    """
    # OPTIMIZATION: Check if this is a static caption (no animation effects)
    # Templates with animation effects that need frame-by-frame rendering
    animated_templates = ["creator_clean", "neon_heartbeat"]
    
    # If the template is not animated and there's no word highlighting, we can render it once
    is_static_caption = (
        template.lower() not in animated_templates and 
        current_word is None and
        not is_profane
    )
    
    # Use a cache for static captions to avoid re-rendering the same text
    if not hasattr(create_caption_clip, 'static_caption_cache'):
        create_caption_clip.static_caption_cache = {}
    
    # Create a cache key based on the parameters that affect the appearance
    if is_static_caption:
        cache_key = (
            text, 
        ...
        # Check if we already have this caption in the cache
        if cache_key in create_caption_clip.static_caption_cache:
            # Reuse the cached image, but create a new clip with the correct duration
            cached_image_path = create_caption_clip.static_caption_cache[cache_key]
            from moviepy.editor import ImageClip
            clip = ImageClip(cached_image_path, transparent=True).set_duration(duration)
    ...
    try:
        ...
        if os.path.exists(font_path):
            ...
            if font_key in _font_cache:
                ...
                print(f"Loaded and cached font: {font_path} at high resolution size: {high_res_font_size}")
            ...
            if available_fonts:
                ...
                # Use font cache for fallback font too
                font_key = f"{fallback_font_path}_{high_res_font_size}"
                if font_key in _font_cache:
                    font = _font_cache[font_key]
                    print(f"Using cached fallback font: {fallback_font_path}")
                else:
                    font = ImageFont.truetype(fallback_font_path, high_res_font_size)
                    _font_cache[font_key] = font
                    print(f"Loaded and cached fallback font: {fallback_font_path}")
    ...
    # For static captions, use a more persistent filename based on a hash of the text
    if is_static_caption:
        # Create a hash of the text to use in the filename
        import hashlib
        text_hash = hashlib.md5(text.encode('utf-8')).hexdigest()[:10]
        temp_filename = os.path.join(temp_dir, f"static_caption_{template.lower()}_{text_hash}.png")
    else:
        # For dynamic captions, use a random UUID as before
    ...
    # If this is a static caption, add it to the cache
    if is_static_caption:
        cache_key = (
            text, 
            video_size[0], video_size[1], 
        ...
        create_caption_clip.static_caption_cache[cache_key] = temp_filename
        
        # Don't delete static caption files - they'll be reused
        temp_file = type('obj', (object,), {'name': temp_filename})
    else:
        # For dynamic captions, we'll delete the file after creating the clip
        temp_file = type('obj', (object,), {'name': temp_filename})
    ...
    # Only delete temporary files for dynamic captions
    if not is_static_caption:
        try:
            os.unlink(temp_file.name)
        except Exception as e:
            print(f"Warning: Could not delete temporary caption file: {e}")
    else:
        # For static captions, manage the cache size
        # If the cache gets too large, remove the oldest entries
        if not hasattr(create_caption_clip, 'cleanup_counter'):
            create_caption_clip.cleanup_counter = 0
        
        # Increment the counter
        create_caption_clip.cleanup_counter += 1
        
        # Check if we need to clean up (every 100 static captions)
        if create_caption_clip.cleanup_counter >= 100:
            create_caption_clip.cleanup_counter = 0
            
            # If we have more than 500 cached captions, remove the oldest ones
            if len(create_caption_clip.static_caption_cache) > 500:
                print(f"Cleaning up static caption cache (size: {len(create_caption_clip.static_caption_cache)})")
                
                # Get a list of all cache entries
                cache_entries = list(create_caption_clip.static_caption_cache.items())
                
                # Sort by file modification time (oldest first)
                import os
                cache_entries.sort(key=lambda x: os.path.getmtime(x[1]) if os.path.exists(x[1]) else 0)
                
                # Remove the oldest 100 entries
                for i in range(100):
                    if i < len(cache_entries):
                        key, filepath = cache_entries[i]
                        if os.path.exists(filepath):
                            try:
                                os.unlink(filepath)
                            except Exception as e:
                                print(f"Warning: Could not delete cached caption file: {e}")
                        # Remove from cache dictionary
                        create_caption_clip.static_caption_cache.pop(key, None)
                
                print(f"Static caption cache cleaned up (new size: {len(create_caption_clip.static_caption_cache)})")
...
def add_captions_to_video(video_path, output_path, phrases, template="minimal_white", highlight_color=None, add_watermark=False, user_plan="free"):
    ...
    # Process phrases in batches of 50 to avoid memory issues with very long videos
    batch_size = 50
    
    # OPTIMIZATION 10: Prepare a font cache to avoid reloading fonts
    font_cache = {}
    
    # OPTIMIZATION: Cache for TextClips to minimize creation
    # Structure: {text: {current_word: clip}} for word-level highlighting
    # or {text: clip} for standard captions
    caption_clip_cache = {}
    ...
    for i in range(0, len(phrases), batch_size):
        ...
        if template.lower() in ["yellow_impact", "bold_white", "bold_green"]:
            # For Word by Word templates, we need to process each word individually
            # but we can still reuse clips for repeated words
            word_clip_cache = {}
            ...
            for phrase in batch_phrases:
                if "word_timestamps" in phrase and len(phrase["word_timestamps"]) > 0:
                    for word_data in phrase["word_timestamps"]:
                        ...
                        # Create a cache key for this word
                        cache_key = f"{word}_{is_profane}"
                        
                        # Check if we already have this word in the cache
                        if cache_key not in word_clip_cache:
                            # Create a caption clip for just this word with increased font size
                            word_clip = create_caption_clip(
                                word,  # Just the current word
                                video_size,
                                word_duration,
                            ...
                            # Store in cache
                            word_clip_cache[cache_key] = word_clip
                        else:
                            # Reuse the cached clip
                            word_clip = word_clip_cache[cache_key]
                    ...
                    # Fallback for phrases without word timestamps
                    start_time = phrase["start"]
                    end_time = phrase["end"]
                    duration = end_time - start_time
                    
                    # Use the standard caption approach for this phrase
                    text = phrase["text"]
                    
                    # Check if we already have this text in the cache
                    if text not in caption_clip_cache:
                        ...
                        # Store in cache
                        caption_clip_cache[text] = caption
                    else:
                        # Reuse the cached clip
                        caption = caption_clip_cache[text]
                    
                    caption = caption.set_start(start_time).set_duration(duration)
                    caption_clips.append(caption)
        else:
            # For other templates, we can optimize by reusing clips for phrases with the same text
            # and for word-level highlighting, we can reuse clips for the same phrase+current_word combination
            ...
            for phrase in batch_phrases:
                ...
                if "word_timestamps" in phrase and len(phrase["word_timestamps"]) > 0:
                    # Create a nested dictionary for this phrase if it doesn't exist
                    if text not in caption_clip_cache:
                        caption_clip_cache[text] = {}
                    
                    # Process each word with its own timing
                    for word_data in phrase["word_timestamps"]:
                        word = word_data["word"]
                        word_start = word_data["start"]
                        word_end = word_data["end"]
                        ...
                        # Create a cache key for this word in this phrase
                        cache_key = f"{word}_{is_profane}"
                        
                        # Check if we already have this word highlighted in this phrase
                        if cache_key not in caption_clip_cache[text]:
                            ...
                            # Store in cache
                            caption_clip_cache[text][cache_key] = caption
                        else:
                            # Reuse the cached clip
                            caption = caption_clip_cache[text][cache_key]
                        
                        # Set the timing for this specific word
                        caption = caption.set_start(word_start).set_duration(word_duration)
                        caption_clips.append(caption)
                else:
                    # Standard caption for phrases without word timestamps
                    # Check if we already have this text in the cache
                    if text not in caption_clip_cache or not isinstance(caption_clip_cache[text], dict):
                        # Create a new caption clip
                        caption = create_caption_clip(
                            text,
                            video_size,
                            duration,
                    ...
                    # Set the timing for this phrase
                    caption = caption.set_start(start_time).set_duration(duration)
                    caption_clips.append(caption)
    ...
    try:
        ...
        render_threads = min(num_cores, 4)  # Use up to 4 cores, but not more than available
        print(f"Using {render_threads} cores for video rendering")
        ...
        if not use_gpu:
            try:
                ...
                print("Intel QuickSync not detected")
