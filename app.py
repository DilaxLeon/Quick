...
# Environment variables for Whisper optimization
WHISPER_MODEL_SIZE = os.environ.get('WHISPER_MODEL_SIZE', 'base')  # tiny, tiny.en, base, small, medium, large
WHISPER_USE_FP16 = os.environ.get('WHISPER_USE_FP16', 'true').lower() == 'true'  # Use fp16 for RTX 4090 speedup
WHISPER_PRELOAD_MODEL = os.environ.get('WHISPER_PRELOAD_MODEL', 'false').lower() == 'true'  # Preload model at startup
WHISPER_DEVICE = os.environ.get('WHISPER_DEVICE', 'auto')  # auto, cpu, cuda

# --- FFmpeg Configuration ---
# Environment variables for FFmpeg optimization
FFMPEG_USE_HARDWARE_ACCEL = os.environ.get('FFMPEG_USE_HARDWARE_ACCEL', 'true').lower() == 'true'  # Use NVENC on RTX 4090
FFMPEG_FALLBACK_TO_MOVIEPY = os.environ.get('FFMPEG_FALLBACK_TO_MOVIEPY', 'true').lower() == 'true'  # Fallback to MoviePy if FFmpeg fails
FFMPEG_QUALITY = os.environ.get('FFMPEG_QUALITY', '23')  # CRF/CQ quality (lower = better quality)

print(f"Whisper Configuration:")
print(f"  Model Size: {WHISPER_MODEL_SIZE}")
print(f"  Use FP16: {WHISPER_USE_FP16}")
print(f"  Preload Model: {WHISPER_PRELOAD_MODEL}")
print(f"  Device: {WHISPER_DEVICE}")

print(f"FFmpeg Configuration:")
print(f"  Hardware Acceleration: {FFMPEG_USE_HARDWARE_ACCEL}")
print(f"  Fallback to MoviePy: {FFMPEG_FALLBACK_TO_MOVIEPY}")
print(f"  Quality (CRF/CQ): {FFMPEG_QUALITY}")

# Global variable to store the Whisper model
# We'll load it on demand or at startup based on configuration
model = None

def get_whisper_device():
    """Determine the best device for Whisper based on configuration and availability"""
...
def load_whisper_model(model_size=None, device=None, fp16=None):
    """Load Whisper model with optimized settings"""
    model_size = model_size or WHISPER_MODEL_SIZE
    device = device or get_whisper_device()
    fp16 = fp16 if fp16 is not None else WHISPER_USE_FP16
    
    print(f"Loading Whisper model '{model_size}' on device '{device}' with fp16={fp16}")
    ...
    try:
        ...
        model = whisper.load_model(
            ...
            in_memory=True  # Keep model in memory for faster subsequent loads
        )
        ...
        if fp16 and device == 'cuda':
            try:
                model = model.half()
                print("Applied fp16 optimization for faster inference")
            except Exception as e:
                print(f"Warning: Could not apply fp16 optimization: {e}")
        ...
        # Fallback to base model on CPU if the requested configuration fails
        if model_size != 'base' or device != 'cpu':
...
def get_model_recommendations():
    """Get model size recommendations based on use case"""
    return {
        'tiny': 'Fastest, lowest accuracy. Good for real-time or very short clips.',
        'tiny.en': 'Fastest for English-only content (30-50% faster than tiny).',
        'base': 'Balanced speed/accuracy. Good default choice.',
        'small': 'Better accuracy, slower than base. Good for important content.',
        'medium': 'High accuracy, significantly slower. For professional use.',
        'large': 'Highest accuracy, very slow. For critical transcriptions.',
        'current': f'Currently using: {WHISPER_MODEL_SIZE}'
    }
...
def api_status():
    ...
    if model_loaded:
        try:
            # Try to get model device info
            model_info['device'] = str(model.device) if hasattr(model, 'device') else 'unknown'
            model_info['is_fp16'] = hasattr(model, 'dtype') and 'float16' in str(model.dtype)
        except:
            model_info['device'] = 'unknown'
            model_info['is_fp16'] = False
    ...
    return jsonify({
        ...
        'whisper_config': {
            ...
            'use_fp16': WHISPER_USE_FP16,
            'preload_model': WHISPER_PRELOAD_MODEL,
            'device': WHISPER_DEVICE,
        ...
        'ffmpeg_config': {
            'use_hardware_accel': FFMPEG_USE_HARDWARE_ACCEL,
            'fallback_to_moviepy': FFMPEG_FALLBACK_TO_MOVIEPY,
...
@app.route('/api/whisper/config', methods=['GET'])
@log_route_details
def api_whisper_config():
    """Get current Whisper configuration and model status"""
    global model
    ...
    cuda_info = {'available': False, 'device_count': 0, 'devices': []}
    try:
        import torch
        cuda_info['available'] = torch.cuda.is_available()
        if cuda_info['available']:
            cuda_info['device_count'] = torch.cuda.device_count()
            cuda_info['devices'] = [torch.cuda.get_device_name(i) for i in range(cuda_info['device_count'])]
            cuda_info['current_device'] = torch.cuda.current_device()
    except ImportError:
        cuda_info['torch_available'] = False
    ...
    return jsonify({
        ...
        'config': {
            ...
            'preload_model': WHISPER_PRELOAD_MODEL,
            'device': WHISPER_DEVICE,
            'detected_device': get_whisper_device()
        },
        'model_status': {
            'loaded': model_loaded,
            'info': model_info
        ...
        'cuda_info': cuda_info,
        'recommendations': get_model_recommendations(),
        'available_models': ['tiny', 'tiny.en', 'base', 'small', 'medium', 'large'],
        'optimization_tips': {
            'rtx_4090': 'Use fp16=True for 30-50% speedup on RTX 4090',
            'english_only': 'Use tiny.en or base.en for English-only content (faster)',
            'preload': 'Set WHISPER_PRELOAD_MODEL=true to avoid loading delays',
            'model_size': 'tiny: fastest, large: most accurate. base is good balance.'
        }
    })

@app.route('/api/whisper/reload', methods=['POST'])
@log_route_details
def api_whisper_reload():
    ...
    try:
        ...
        if old_model is not None:
            try:
                ...
                import gc
                gc.collect()
                if WHISPER_DEVICE == 'cuda' or get_whisper_device() == 'cuda':
                    try:
                        import torch
        ...
        return jsonify({
            'success': False,
            'error': str(e),
            'message': 'Failed to reload Whisper model'
        }), 500
...
def convert_to_9_16_ratio(input_path, output_path):
    """Convert video to 9:16 aspect ratio using FFmpeg for optimal performance"""
    import subprocess
    import json
    
    print(f"Converting video to 9:16 using FFmpeg: {input_path} -> {output_path}")
    
    try:
        # First, get video information using ffprobe
        probe_cmd = [
            'ffprobe', '-v', 'quiet', '-print_format', 'json', '-show_streams',
            input_path
        ]
        
        print(f"Getting video info with: {' '.join(probe_cmd)}")
        ...
        print(f"Video dimensions: {width}x{height}, aspect ratio: {current_ratio:.3f}")
        print(f"Target aspect ratio: {target_ratio:.3f}")
        
        # If already 9:16 or taller, just copy the file
        if current_ratio <= target_ratio:
            print("Video already has correct aspect ratio, copying file")
        ...
        print(f"Cropping to: {new_width}x{height} with x_offset: {x_offset}")
        
        # Try hardware-accelerated encoding first (NVENC for RTX 4090)
        success = False
        
        # NVENC hardware acceleration attempt (if enabled)
        if FFMPEG_USE_HARDWARE_ACCEL and not success:
            try:
                print("Attempting NVENC hardware acceleration...")
                nvenc_cmd = [
                    'ffmpeg', '-y', '-i', input_path,
                    '-vf', f'crop={new_width}:{height}:{x_offset}:0',
                    '-c:v', 'h264_nvenc',
                    '-preset', 'p4',  # Fastest preset for NVENC
                    '-tune', 'hq',   # High quality
                    '-rc', 'vbr',    # Variable bitrate
                    '-cq', FFMPEG_QUALITY,  # Quality level from config
                ...
                print(f"NVENC command: {' '.join(nvenc_cmd)}")
                subprocess.run(nvenc_cmd, check=True, capture_output=True)
                success = True
                print("✅ NVENC hardware acceleration successful!")
                
            except subprocess.CalledProcessError as e:
                print(f"❌ NVENC failed: {e}")
                print(f"NVENC stderr: {e.stderr.decode() if e.stderr else 'No stderr'}")
        
        # Fallback to CPU encoding with optimized settings
        if not success:
            try:
                print("Falling back to optimized CPU encoding...")
                cpu_cmd = [
                    'ffmpeg', '-y', '-i', input_path,
                    '-vf', f'crop={new_width}:{height}:{x_offset}:0',
                    '-c:v', 'libx264',
                    '-preset', 'veryfast',  # Fast encoding
                    '-crf', FFMPEG_QUALITY,  # Quality from config
                    '-c:a', 'aac',
                    '-b:a', '128k',
                    '-pix_fmt', 'yuv420p',
                    '-profile:v', 'high',
                ...
                print(f"CPU command: {' '.join(cpu_cmd)}")
                ...
                print(f"CPU stderr: {e.stderr.decode() if e.stderr else 'No stderr'}")
        
        # Final fallback to MoviePy if FFmpeg fails (if enabled)
        if not success and FFMPEG_FALLBACK_TO_MOVIEPY:
            print("⚠️ FFmpeg failed, falling back to MoviePy...")
            return convert_to_9_16_ratio_moviepy(input_path, output_path)
        elif not success:
            raise RuntimeError("FFmpeg conversion failed and MoviePy fallback is disabled")
        
        # Verify output file was created
        if not os.path.exists(output_path):
            raise FileNotFoundError(f"Output file was not created: {output_path}")
        
        print(f"✅ Video conversion completed successfully: {output_path}")
        return output_path
        
    except Exception as e:
        print(f"❌ FFmpeg conversion failed: {e}")
        if FFMPEG_FALLBACK_TO_MOVIEPY:
            print("⚠️ Falling back to MoviePy...")
            return convert_to_9_16_ratio_moviepy(input_path, output_path)
        else:
            print("❌ MoviePy fallback is disabled")
            raise

def convert_to_9_16_ratio_moviepy(input_path, output_path):
    """Fallback MoviePy implementation for 9:16 conversion"""
    print("Using MoviePy fallback for 9:16 conversion")
...
def transcribe_video(video_path, filter_profanity=False, preloaded_model=None):
    ...
    # If no model is available, load a new one with optimized settings
    else:
        print("Loading new Whisper model with optimized settings...")
        try:
    ...
    print(f"Transcribing video with {'preloaded' if preloaded_model is not None else 'optimized'} model")
    print(f"Using model: {WHISPER_MODEL_SIZE}, fp16: {WHISPER_USE_FP16}")
    
    # Optimized transcription parameters
    transcribe_options = {
        'word_timestamps': True,
        'task': 'transcribe',  # Explicitly use transcribe task, not translate
        'language': None,  # Auto-detect language
        'verbose': False,  # Reduce verbose output to minimize log spam
    }
    
    # Add fp16 option if supported and enabled
    if WHISPER_USE_FP16:
        transcribe_options['fp16'] = True
    
    result = transcription_model.transcribe(video_path, **transcribe_options)
...
def create_caption_clip(text, video_size, duration, font_size=40, template="minimal_white", current_word=None, current_word_color=None, is_profane=False, custom_text_color=None, custom_highlight_color=None, custom_font=None):
    ...
    try:
        ...
        if os.path.exists(font_path):
            ...
            print(f"Successfully loaded font: {font_path} at high resolution size: {high_res_font_size}")
        else:
            ...
            if available_fonts:
                ...
                quality_multiplier = 2.0  # Render at 2x size for better quality
                high_res_font_size = int(font_size * quality_multiplier)
                font = ImageFont.truetype(fallback_font_path, high_res_font_size)
                highlighted_font = font
            else:
    ...
    # Position captions at 70% down the screen (similar to the original positioning)
    is_vertical = video_size[0] / video_size[1] <= 0.65  # Approximately 9:16 or narrower
    caption_position_ratio = 0.6 if is_vertical else 0.7
