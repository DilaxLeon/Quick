...
def create_caption_clip(text, video_size, duration, font_size=40, template="minimal_white", current_word=None, current_word_color=None, is_profane=False, custom_text_color=None, custom_highlight_color=None, custom_font=None, caption_position=None):
    ...
    Args:
        ...
        caption_position: Custom vertical position ratio (0.0 to 1.0) for captions
    ...
    # If a custom caption position is provided, use it
    if caption_position is not None:
        # Ensure the position is within valid range (0.0 to 1.0)
        caption_position_ratio = max(0.1, min(0.9, caption_position))
    else:
        # Default positions: 60% for vertical videos, 70% for horizontal
...
def process_video(input_path, options=None, preloaded_model=None):
    ...
    try:
        ...
        # Get caption position from options if available
        caption_position = options.get("caption_position") if options else None
        print(f"Caption position: {caption_position}")
        add_captions_to_video(converted_path, output_path, phrases, template=template, highlight_color=highlight_color, add_watermark=add_watermark, user_plan=user_plan, caption_position=caption_position)
...
def add_captions_to_video(video_path, output_path, phrases, template="minimal_white", highlight_color=None, add_watermark=False, user_plan="free", caption_position=None):
...
@app.route('/api/adjust_caption_position', methods=['POST', 'OPTIONS'])
@log_route_details
def adjust_caption_position():
    """API endpoint to adjust the vertical position of captions in a video"""
    from flask import jsonify, request
    ...
    try:
        ...
        caption_position = request.form.get('caption_position', None)
        template_id = request.form.get('template', 'minimal_white')
        highlight_color = request.form.get('highlight_color', None)
        user_plan = request.form.get('user_plan', 'free').lower()
        
        # Log the request parameters for debugging
        print(f"Caption position adjustment request:")
        ...
        if caption_position:
            try:
                ...
                # Ensure position is within valid range (0.1 to 0.9)
                caption_position = max(0.1, min(0.9, caption_position))
                print(f"Using custom caption position: {caption_position}")
            except ValueError:
                return jsonify({'error': 'Invalid caption position value. Must be a number between 0.1 and 0.9.', 'success': False}), 400
        else:
            return jsonify({'error': 'Caption position is required.', 'success': False}), 400
        
        # Validate required parameters
        if not video_path:
            return jsonify({'error': 'Video path is required.', 'success': False}), 400
        if not transcription_path:
            return jsonify({'error': 'Transcription path is required.', 'success': False}), 400
        
        try:
            # Continue with the same reprocessing logic as the reprocess_video function
            # but with the caption position parameter
            return reprocess_video(caption_position=caption_position)
        except Exception as inner_e:
            import traceback
            error_trace = traceback.format_exc()
            print(f"Error in reprocess_video: {str(inner_e)}")
            print(error_trace)
            return jsonify({
                'error': f"Error processing video: {str(inner_e)}",
                'success': False,
                'details': error_trace
            }), 500
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error in adjust_caption_position: {str(e)}")
        print(error_trace)
        return jsonify({
...
@log_route_details
def reprocess_video(caption_position=None):
    ...
    try:
        ...
        # Log the request parameters for debugging
        print(f"Reprocess video request:")
        print(f"  - Video path: {video_path}")
        print(f"  - Transcription path: {transcription_path}")
        print(f"  - Template: {template_id}")
        print(f"  - Highlight color: {highlight_color}")
        print(f"  - User plan: {user_plan}")
        print(f"  - Caption position (from parameter): {caption_position}")
        
        # If caption_position wasn't passed as a parameter, check if it's in the form data
        if caption_position is None:
            caption_position_str = request.form.get('caption_position', None)
            if caption_position_str:
                try:
                    caption_position = float(caption_position_str)
                    # Ensure position is within valid range (0.1 to 0.9)
                    caption_position = max(0.1, min(0.9, caption_position))
                    print(f"Using custom caption position from form data: {caption_position}")
                except ValueError:
                    print(f"Invalid caption position value in form data: {caption_position_str}")
        ...
        if not video_path or not transcription_path:
            return jsonify({'error': 'Missing video_path or transcription_path', 'success': False}), 400
        ...
        # Caption position is now handled at the beginning of the function
        print(f"Using caption position: {caption_position}")
        
        add_captions_to_video(temp_video_path, output_path, phrases, template=template_id, highlight_color=highlight_color, 
                             add_watermark=add_watermark, user_plan=user_plan, caption_position=caption_position)
        ...
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error in reprocess_video: {str(e)}")
        print(error_trace)
        return jsonify({
            'error': f"Server error: {str(e)}",
            'success': False,
