...
# Disable progress bar for cleaner logs, especially with large videos
configure_moviepy_all(temp_dir="/app/tmp", disable_progress_bar=True)
...
def process_video(input_path, options=None, preloaded_model=None):
    ...
    # Check if this is a large video (> 30 minutes) to optimize processing
    is_large_video = False
    try:
        # Just get the duration without loading the entire video
        import subprocess
        import json
        ...
        is_large_video = duration > 1800  # 30 minutes
        print(f"Video duration: {duration} seconds, is_large_video: {is_large_video}")
        log_debug(f"Video duration: {duration} seconds, is_large_video: {is_large_video}")
    except Exception as e:
        print(f"Error checking video duration: {e}, proceeding with standard processing")
        log_debug(f"Error checking video duration: {e}, proceeding with standard processing")
    
    # For large videos, disable progress bar globally to reduce log spam
    if is_large_video:
        from moviepy.config import change_settings
        change_settings({"PROGRESS_BAR": False})
        print("Disabled progress bar for large video processing")
        log_debug("Disabled progress bar for large video processing")
    ...
    if user_plan.lower() in ["basic", "pro"]:
        ...
        try:
            ...
            # For very high resolutions or large videos, use a faster preset to avoid memory issues
            if new_width >= 3840 or new_height >= 3840 or is_large_video:
                actual_preset = "veryfast"  # Use fastest preset for high resolutions or large videos
                print(f"Using fastest preset 'veryfast' for high resolution or large video")
            else:
                actual_preset = preset
                print(f"Using standard preset '{preset}' for normal resolution and duration")
            ...
            # Additional optimizations for large videos
            write_kwargs = {
                "codec": "libx264",
                "audio_codec": "aac",
            ...
            # For large videos, disable logger to reduce output spam
            if is_large_video:
                write_kwargs["logger"] = None
                print("Disabled logger for large video to reduce output spam")
            
            original_clip.write_videofile(original_video_path, **write_kwargs)
        except Exception as e:
            ...
            try:
                ...
                if new_width > 1920 or new_height > 1920:
                    ...
                    fallback_kwargs = {
                        ...
                        "audio_bitrate": "128k",
                        "bitrate": "4000k",
                        "preset": "veryfast" if is_large_video else "fast",
                        "temp_audiofile": os.path.join("/app/tmp", f"temp-audio-{uuid.uuid4().hex}.m4a"),
                        "remove_temp": True,
                    ...
                    # For large videos, disable logger to reduce output spam
                    if is_large_video:
                        fallback_kwargs["logger"] = None
                        print("Using veryfast preset and disabled logger for large video fallback")
                    
                    resized_clip.write_videofile(original_video_path, **fallback_kwargs)
                    resized_clip.close()
                else:
                    ...
                    # For large videos, disable logger to reduce output spam
                    if is_large_video:
                        safe_kwargs["logger"] = None
                        print("Using veryfast preset and disabled logger for large video safe settings")
                    
                    original_clip.write_videofile(original_video_path, **safe_kwargs)
        ...
        try:
            ...
            # For large videos, disable logger to reduce output spam
            if is_large_video:
                free_kwargs["logger"] = None
                print("Using veryfast preset and disabled logger for large video (free plan)")
            
            resized_clip.write_videofile(original_video_path, **free_kwargs)
            resized_clip.close()
            original_clip.close()
        except Exception as e:
    ...
    try:
        ...
        # Pass the is_large_video flag to optimize caption processing
        add_captions_to_video(converted_path, output_path, phrases, template=template, highlight_color=highlight_color, add_watermark=add_watermark, user_plan=user_plan, is_large_video=is_large_video)
...
def add_captions_to_video(video_path, output_path, phrases, template="minimal_white", highlight_color=None, add_watermark=False, user_plan="free", is_large_video=False):
    ...
    Args:
        ...
        user_plan: The user's subscription plan ("free", "basic", or "pro")
        is_large_video: Whether this is a large video (> 30 minutes) for optimization
    """
    # For large videos, disable progress bar to reduce log spam
    if is_large_video:
        from moviepy.config import change_settings
        change_settings({"PROGRESS_BAR": False})
        print("Disabled progress bar for large video processing")
    ...
    try:
        ...
        # For large videos, use faster preset to reduce processing time
        if is_large_video and preset != "veryfast":
            print(f"Large video detected, adjusting preset from {preset} to veryfast")
            preset = "veryfast"
            
        # For large videos, use a more efficient writing process
        write_params = {
            "codec": "libx264",
            "audio_codec": "aac",
            "audio_bitrate": audio_bitrate,
        ...
        print(f"Writing video to {output_path} with preset: {preset}")
