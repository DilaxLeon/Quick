...
def censor_content(audio_path, segments, output_audio_path=None):
    ...
    if output_audio_path is None:
        output_audio_path = os.path.join(os.path.dirname(audio_path), "censored_audio.wav")
    ...
    if filter_profanity:
        try:
            ...
            result = censor_content(
                temp_audio_path, 
                segments, 
                output_audio_path=os.path.join(temp_dir, "censored_audio.wav")
            )
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        converted_path = os.path.join(app.config['UPLOAD_FOLDER'], f"converted_{os.path.basename(file_path)}")
...
def reprocess_video():
    ...
    try:
        ...
        if 'video' in request.files:
            ...
            temp_upload_path = os.path.join(UPLOAD_FOLDER, f"upload_{uuid.uuid4().hex}_{secure_filename(file.filename)}")
        ...
        # Create a temporary copy of the original video without captions
        temp_video_path = os.path.join(UPLOAD_FOLDER, f"temp_{uuid.uuid4().hex}_{video_filename}")
