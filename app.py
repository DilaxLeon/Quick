...
def get_processed_file(filename):
    try:
        ...
        print(f"Original filename parameter: {filename}")
        print(f"File exists check: {os.path.exists(os.path.join(PROCESSED_FOLDER, decoded_filename))}")
        ...
        if USE_R2_STORAGE and r2_client:
            ...
            download = request.args.get('download', 'false').lower() == 'true'
            file_path = os.path.join(PROCESSED_FOLDER, decoded_filename)
            print(f"Looking for file at: {file_path}")
            print(f"PROCESSED_FOLDER: {PROCESSED_FOLDER}")
            
            if not os.path.exists(file_path):
                print(f"File not found: {decoded_filename}")
                print(f"Files in processed folder: {os.listdir(PROCESSED_FOLDER) if os.path.exists(PROCESSED_FOLDER) else 'Folder does not exist'}")
                return f"File not found: {decoded_filename}", 404
            
            print(f"Serving file: {decoded_filename}")
            return send_from_directory(PROCESSED_FOLDER, decoded_filename, as_attachment=download)
