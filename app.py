...
# Import GPU-accelerated video processor
from gpu_video_processor import convert_to_9_16_ratio_gpu, get_video_info as get_gpu_video_info
...
def convert_to_9_16_ratio(input_path, output_path):
    ...
    This function now uses GPU acceleration for significantly better performance
    compared to the previous MoviePy CPU-bound implementation.
    """
    try:
        print(f"Converting video to 9:16 ratio using GPU acceleration: {input_path} -> {output_path}")
        
        # Use GPU-accelerated conversion with balanced quality
        result_path = convert_to_9_16_ratio_gpu(input_path, output_path, quality="balanced")
        
        print(f"GPU-accelerated conversion completed successfully: {result_path}")
        return result_path
        
    except Exception as gpu_error:
        print(f"GPU conversion failed: {str(gpu_error)}")
        print("Falling back to MoviePy CPU-based conversion...")
        
        # Fallback to original MoviePy implementation
        try:
            ...
            print(f"Fallback conversion - Input: {width}x{height}, ratio: {current_ratio:.3f}")
            ...
            print(f"Cropping to: {new_width}x{height} (offset: {x1}, 0)")
            ...
            print("Fallback MoviePy conversion completed successfully")
            return output_path
            
        except Exception as fallback_error:
            print(f"Fallback conversion also failed: {str(fallback_error)}")
            raise RuntimeError(f"Both GPU and CPU conversion failed. GPU error: {str(gpu_error)}, CPU error: {str(fallback_error)}")
...
def process_video(input_path, options=None, preloaded_model=None):
    ...
    if enable_speaker_tracking and (user_plan.lower() == "pro" or user_plan.lower() == "basic"):
        print("📍 STEP 1: Speaker Tracking & Diarization")
        step_start_time = time.time()
        try:
            ...
            # Log timing for speaker tracking
            speaker_tracking_time = time.time() - step_start_time
            print(f"⏱️  STEP 1 COMPLETED: Speaker Tracking took {speaker_tracking_time:.2f} seconds")
            ...
            print(f"⏱️  STEP 1 FAILED: Speaker Tracking failed after {speaker_tracking_time:.2f} seconds")
    ...
    print("📍 STEP 2: Convert to 9:16 Aspect Ratio")
    step_start_time = time.time()
    ...
    try:
        ...
        conversion_time = time.time() - step_start_time
        print(f"Video conversion completed successfully")
        print(f"⏱️  STEP 2 COMPLETED: 9:16 Conversion took {conversion_time:.2f} seconds")
        ...
        import traceback
        traceback.print_exc()
        print(f"⏱️  STEP 2 FAILED: 9:16 Conversion failed after {conversion_time:.2f} seconds")
        raise
    
    # Step 3: Transcribe the video with optional profanity filtering
    print("📍 STEP 3: Video Transcription with Whisper")
    step_start_time = time.time()
