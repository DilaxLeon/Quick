...
def get_video_mime_type(filename):
    """Get the appropriate MIME type for video files"""
    ...
    return video_mime_types.get(file_extension), file_extension in video_mime_types

def create_video_response(folder_path, filename, as_attachment=False):
    """Create a Flask response for video files with proper headers"""
    decoded_filename = urllib.parse.unquote(filename)
    mime_type, is_video = get_video_mime_type(decoded_filename)
    
    if is_video:
        print(f"Creating video response for {decoded_filename} with MIME type: {mime_type}")
        ...
        # Add video streaming headers if not downloading
        if not as_attachment:
            response.headers['Accept-Ranges'] = 'bytes'
            response.headers['Content-Type'] = mime_type
            response.headers['Cache-Control'] = 'public, max-age=3600'
            
            # Ensure no download prompt for video viewing
            if 'Content-Disposition' in response.headers:
                del response.headers['Content-Disposition']
        
        print(f"Video response headers: {dict(response.headers)}")
        return response
    else:
        # For non-video files, use standard handling
        return send_from_directory(folder_path, decoded_filename, as_attachment=as_attachment)
...
def get_processed_file(filename):
    try:
        ...
        if USE_R2_STORAGE and r2_client:
            ...
            # Use helper function to create proper video response
            return create_video_response(app.config['PROCESSED_FOLDER'], decoded_filename, as_attachment=download)
...
def serve_processed_file(filename):
    ...
    try:
        ...
        # Use helper function to create proper video response
        return create_video_response(app.config['PROCESSED_FOLDER'], decoded_filename, as_attachment=False)
    except Exception as e:
...
def view_processed_file(filename):
    """View a processed file with proper MIME type for videos"""
    try:
        # Use helper function to create proper video response
        return create_video_response(app.config['PROCESSED_FOLDER'], filename, as_attachment=False)
    except Exception as e:
        print(f"Error in view_processed_file: {str(e)}")
        return f"Error viewing file: {str(e)}", 500

@app.route('/debug/mime-type/<filename>')
@log_route_details
def debug_mime_type(filename):
    """Debug endpoint to test MIME type detection"""
    try:
        decoded_filename = urllib.parse.unquote(filename)
        mime_type, is_video = get_video_mime_type(decoded_filename)
        ...
        return jsonify({
            ...
            'mime_type': mime_type,
            'is_video': is_video,
            'file_extension': decoded_filename.lower().split('.')[-1] if '.' in decoded_filename else ''
        })
    except Exception as e:
