...
# Dictionary to hold the status of background tasks
tasks = {}
...
def get_status(task_id):
    ...
    response = {'status': task.get('status', 'unknown')}
    if 'message' in task:
...
def process_video_task(task_id, file_path, options):
    """
    Processes a video in a background thread.
    Updates the global `tasks` dictionary with status and results.
    """
    with app.app_context():
        try:
            tasks[task_id]['status'] = 'processing'
            log_debug(f"Task {task_id}: Starting video processing for {file_path}")

            # Create a dedicated debug log for this task
            debug_log_path = os.path.join(PROCESSED_FOLDER, f"api_debug_log_{task_id}.txt")
            def log_task_debug(message):
                try:
                    with open(debug_log_path, 'a') as f:
                        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                    ...
                    print(f"Task {task_id}: Error writing to task debug log: {str(e)}")

            log_task_debug("=== BACKGROUND TASK STARTED ===")
            log_task_debug(f"Options: {options}")

            # Call the main processing function
            output_filename, original_video_filename = process_video(file_path, options)
            log_task_debug(f"process_video completed. Output: {output_filename}, Original: {original_video_filename}")

            # --- R2 Uploading or URL Generation ---
            transcription_filename = output_filename.rsplit('.', 1)[0] + '.json'
            final_video_url, final_transcription_url, final_original_video_url = None, None, None

            if USE_R2_STORAGE and r2_client:
                log_task_debug("Uploading artifacts to R2.")
                try:
                    ...
                    log_task_debug("R2 upload complete.")
                except ClientError as e:
                    log_task_debug(f"R2 upload failed: {e}. Falling back to local URLs.")
                    # Fallback URLs will be generated below
            
            if not final_video_url: # If R2 is disabled or failed
                log_task_debug("Generating local URLs.")
            ...
            result = {
                ...
                'speaker_tracking_enabled': options.get('include_speakers'),
                'original_video_url': final_original_video_url
            }

            tasks[task_id]['status'] = 'completed'
            tasks[task_id]['result'] = result
            log_task_debug("Task completed successfully.")

        except Exception as e:
            error_message = f"Error in task {task_id}: {str(e)}"
            ...
            if os.path.exists(file_path):
                try:
                    os.remove(file_path)
                    log_debug(f"Task {task_id}: Cleaned up original file: {file_path}")
                except PermissionError:
                    log_debug(f"Task {task_id}: Warning: Could not remove file {file_path} - it may be in use.")
...
def api_process():
    """
    API endpoint to process a video asynchronously.
    Accepts a video upload, starts a background processing task,
    and returns a task ID for status polling.
    """
    if 'video' not in request.files and 'video_data' not in request.files:
        return jsonify({'error': 'No video file provided'}), 400

    file = request.files.get('video') or request.files.get('video_data')
    ...
    # Perform quick checks before starting the background task
    if not check_video_duration(file_path, user_plan):
        max_duration = MAX_VIDEO_DURATION.get(user_plan, DEFAULT_MAX_DURATION)
        duration_msg = f"{max_duration} seconds" if max_duration < 60 else f"{max_duration // 60} minutes"
    ...
    # Collect options for the processing task
    template_id = request.form.get('template', 'minimal_white')
    highlight_color = request.form.get('highlight_color', None)
    
    # Handle custom templates color extraction
    if template_id.startswith('custom_') and not highlight_color:
        color_match = re.search(r'custom_\d+_[a-z0-9]+_([a-f0-9]{6})', template_id, re.IGNORECASE)
        if color_match:
            highlight_color = f"#{color_match.group(1)}"

    options = {
        'template': template_id,
        'highlight_color': highlight_color,
        'censor_profanity': request.form.get('filter_profanity', 'false').lower() == 'true',
        'user_plan': user_plan,
        'include_speakers': request.form.get('enable_speaker_tracking', 'false').lower() == 'true' and user_plan in ['pro', 'basic']
    }

    # Create and start the background task
    task_id = str(uuid.uuid4())
    tasks[task_id] = {'status': 'pending', 'message': 'Task has been queued.'}
    
    thread = threading.Thread(target=process_video_task, args=(task_id, file_path, options))
    thread.daemon = True
    thread.start()
