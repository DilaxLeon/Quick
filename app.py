...
from gpu_config import gpu_config, get_whisper_device, optimize_model, clear_gpu_cache, write_video_with_gpu_acceleration
...
def transcribe_video(video_path, filter_profanity=False, preloaded_model=None):
    ...
    # Clear GPU cache after transcription to free memory for video processing
    clear_gpu_cache()
...
def process_video(input_path, options=None, preloaded_model=None):
    ...
    if user_plan.lower() in ["basic", "pro"]:
        ...
        try:
            ...
            print(f"Starting to write original video file with GPU acceleration...")
            # Determine quality level based on preset
            quality_level = 'ultra' if actual_preset in ['veryslow', 'slow'] else 'high' if actual_preset == 'medium' else 'medium'
            
            success = write_video_with_gpu_acceleration(
                original_clip,
                original_video_path,
                codec="libx264",
                audio_codec="aac",
            ...
            if not success:
                raise Exception("Failed to write video with both GPU and CPU encoding")
        except Exception as e:
...
def add_captions_to_video(video_path, output_path, phrases, template="minimal_white", highlight_color=None, add_watermark=False, user_plan="free"):
    ...
    try:
        ...
        # Determine quality level based on user plan and preset
        ...
        if not success:
            raise Exception("Failed to write captioned video with both GPU and CPU encoding")
    finally:
