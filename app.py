    ...
    try:
        ...
        if caption_position:
            try:
                ...
                return jsonify({'error': 'Invalid caption position value. Must be a number between 0.1 and 0.9.'}), 400
        else:
            return jsonify({'error': 'Caption position is required.'}), 400
        
        # Continue with the same reprocessing logic as the reprocess_video function
        # but with the caption position parameter
        
        # The rest of the function is identical to reprocess_video
        # so we'll call that function with the caption position parameter
