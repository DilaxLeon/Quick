...
if os.path.exists('/app/local_processed') and not os.path.exists(PROCESSED_FOLDER):
    print("Production environment detected, using /app/local_processed")
    PROCESSED_FOLDER = '/app/local_processed'
    os.makedirs(PROCESSED_FOLDER, exist_ok=True)

if os.path.exists('/app/local_uploads') and not os.path.exists(UPLOAD_FOLDER):
    print("Production environment detected, using /app/local_uploads")
    UPLOAD_FOLDER = '/app/local_uploads'
    os.makedirs(UPLOAD_FOLDER, exist_ok=True)
...
@app.route('/api/process', methods=['POST', 'OPTIONS', 'OPTIONS'])
@log_route_details
def api_process():
    ...
    try:
        ...
        if USE_R2_STORAGE and r2_client:
            ...
            final_video_url = url_for('serve_processed_file', filename=output_filename, _external=True)
            final_transcription_url = url_for('get_transcription', filename=transcription_filename, _external=True)
            final_original_video_url = url_for('serve_processed_file', filename=original_video_filename, _external=True)
